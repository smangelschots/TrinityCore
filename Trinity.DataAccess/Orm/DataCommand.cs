...
using Trinity.DataAccess.Collections;
...
namespace Trinity.DataAccess.Orm
{
        ...
        where T : class
    {
        ...
        {
            return string.Format("{0}_{1}", typeof(T).Name, this.TableName);
        }
        ...
        public virtual string GetTableMap()
        {
            ...
            if (!Manager.TableMaps.TryGetValue(modelName,out tableMap))
            {
                tableMap = new TableMap();
                if (string.IsNullOrEmpty(this.TableName))
                {
                    tableMap.TableName = this.GetTableAttribute();
                    this.TableName = tableMap.TableName;
                }
                else
                {
            ...
            this.Manager.OnAfterTryGetTableMap(this, new ModelCommandAfterGetTableMapEventArgs(tableMap, modelName));
            if (tableMap != null)
            {
        ...
        public IDataCommand<T> ForTableUpdate(TableMap tableMap)
        {
            //this.CommandType = DataCommandType.TableCreateUpdate;
        ...
        {
            var result = this.Manager.ExecuteCommand(this);
            this.Manager.Remove(this);
            Manager.Connection.Close();
            Manager.Connection.Dispose();
            return result;
    ...
    public abstract class DataCommand
    {
        ...
        public DataCommand()
            : this(new List<IParameter>(), DataCommandType.Update)
        {
            this.WhereText = string.Empty;
        }

        public DataCommand(List<IParameter> parameters, DataCommandType commandType)
        {
        ...
        {
            string commandString = string.Format("UPDATE [{0}]", this.TableName) + " SET ";
        ...
        {
            string commandString = string.Format("INSERT INTO [{0}] (", this.TableName);
        ...
        {
            if (!string.IsNullOrEmpty(this.TableName))
                if (this.TableName.Contains("JOIN"))
                    return string.Format(" FROM {0} ", this.TableName);
                else
                    return string.Format(" FROM [{0}] ", this.TableName);
            else
