            ...
            foreach (ICommandResult dataResult in errors)
            {
                foreach (DataError error in dataResult.CommandErrors)
                {
                    if (error.Exception != null)
                        result += string.Format("{0} {1} {2} {3}",error.ErrorType, error.Message, error.Exception.Message, Environment.NewLine);
                    else
                    {
                        result += string.Format("{0} {1} {2}", error.ErrorType, error.Message, Environment.NewLine);
                    }
                }
            }
            return result;
