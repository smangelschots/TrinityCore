...
namespace Trinity.SqlServer
{
    public class SqlModelCommand<T> : DataCommand<T> where T : class
    {
        ...
        public SqlModelCommand(IDbConnection connection, List<IParameter> parameters, DataCommandType commandType)
        ...
        {
            {
                ...
                {
                    newDataParameter = Activator.CreateInstance<DataParameter>();
        ...
        {
            if (Manager.TableMaps.ContainsKey(tableMap.TableName) == false)
            {
                Manager.TableMaps.Add(tableMap.TableName, tableMap);
            }
        }
        ...
        public override string GetTableMap()
        {
            ...
            {
                this.TableName = this.GetTableAttribute();
            }
            else
            {
                //if (typeof(T).Name == this.TableName)
                //    IsMapped = true;
            }

            //TODO Refactor
            var modelName = base.GetTableMapName();
            var modelMapFound = Manager.TableMaps.ContainsKey(modelName);
            if(modelMapFound == false)
            {
                modelName = this.TableName;
                modelMapFound = Manager.TableMaps.ContainsKey(modelName);
               
            }
            if (modelMapFound)
            {
                this.TableMap = Manager.TableMaps[modelName];
                if (this.TableMap.Mapped)
                {
            ...
            if(!modelMapFound)
            {
                if (this.Manager.TableMapFromDatabase)
                {
                    var tableMap = this.TableMap ?? new TableMap();
                    tableMap.TableName = this.TableName;
                    ...
                    using (var conn = new SqlConnection(Manager.ConnectionString))
                    {
                        ...
                        using (var command = new SqlCommand(sqlTableMap, conn))
                        {
                            ...
                            foreach (var column in tableMap.ColumnMaps)
                            {
                                ...
                                {
                                    var item = propertyMap.FirstOrDefault(m => m.ColumnName == column.ColumnName);
            ...
            using (var conn = new SqlConnection(Manager.ConnectionString))
            {
                ...
                using (var keyCommand = new SqlCommand(sqlKeysMap, conn))
                {
                    ...
                    using (var keyReader = keyCommand.ExecuteReader())
                    {
                        ...
                        {
                            var keyMap = new  SqlServerKeyMap();
                            keyMap.KeyType =
        ...
        protected internal void BuildSqlParameters(IDbCommand command)
        {
            ...
            foreach (var dataParameter in this.Parameters)
            {
                ...
                var sqlparameter = dataParameter as DataParameter;
                ...
                if (this.TableMap != null)
                {
                    ...
                    if (column != null)
                    {
                        ...
                        if (col != null)
                        {
                            sqlparameter.SqlDbType = col.DbType;
                            sqlparameter.Size = col.Size;
                            sqlparameter.IsNullable = col.IsNullable;
                            sqlparameter.Size = col.Size;
                            sqlparameter.IsForeignKey = col.IsForeignKey;
                            sqlparameter.IsPrimaryKey = col.IsPrimaryKey;
                        }
                    }
                }
                if (sqlparameter.SqlDbType != SqlDbType.Timestamp)
                {
                        ...
                        if (!string.IsNullOrEmpty(valuestring))
                        {
                            if (valuestring.Length > sqlparameter.Size)
                            {
                                throw new ApplicationException(string.Format("Property {0} value is bigger than database size {1} column name in {2} {3} Value {4} length is {5} column length is {6}"
                                    , sqlparameter.PropertyName, sqlparameter.ColumnName, this.TableName, valuestring, valuestring.Length, sqlparameter.Size, valuestring.Length));
                            }
                        }
                    }
